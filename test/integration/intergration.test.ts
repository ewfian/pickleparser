...
import { PROTOCOL, PROTOCOLS, caller, pythonUnpickle } from './_caller';
...
describe('Pickler', () => {
    describe('Integration with Python Unpickler', () => {
        const PROTOCOLS = [1, 2, 3, 4, 5];

        const integrationTestCases = [
            ['a simple list', [1, 'hello', true], "[1, 'hello', True]"],
            ['a simple dict', { a: 1, b: null }, "{'a': 1, 'b': None}"],
            ['a nested structure', { data: [1, { value: 3.14 }] }, "{'data': [1, {'value': 3.14}]}"],
            ['a structure with a BigInt', { big: 123456789n }, "{'big': 123456789}"],
            [
                'a self-referencing list',
                ((): unknown[] => {
                    const l: unknown[] = [1];
                    l.push(l);
                    return l;
        ...
        const testCases = integrationTestCases.flatMap(([name, value, expectedRepr]) =>
            PROTOCOLS.map((p) => [name, value, p, expectedRepr]),
        );

        it.each(testCases)(
            'correctly generates pickle for %s (protocol %p)',
            async (name, value, protocol, expectedRepr) => {
                const pickler = new Pickler({ protocol: protocol as number });
                const pickledData = pickler.dump(value);

                const pythonRepr = await pythonUnpickle(pickledData);

                if (name === 'a self-referencing list') {
                    expect(pythonRepr).toMatch(/^\[1, <Recursion on list with id=\d+>\]$|^\[1, \[...\]\]$/);
                } else {
                    expect(pythonRepr).toBe(expectedRepr);
                }
            },
        );
    });
